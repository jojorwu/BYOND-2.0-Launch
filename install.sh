...
check_dependencies() {
    ...
    if ! command -v $DOTNET_EXEC &> /dev/null || ! $DOTNET_EXEC --list-runtimes | grep -q "Microsoft.NETCore.App 9."; then
        echo ".NET 9 Runtime could not be found. Installing..."
        ensure_dotnet_install_script
...
for choice in $choices; do
    (
        if [ "$choice" == "BYOND Launch" ]; then
            install_component "BYOND Launch" "dist/Launcher" "Launcher" "Launcher"
        elif [ "$choice" == "BYOND 2.0" ]; then
            install_component "BYOND 2.0" "dist/Client" "BYOND2" "Client"
        fi
